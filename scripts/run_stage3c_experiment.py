#!/usr/bin/env python3
"""Run a Stage 3C sparse IRT4 adaptation experiment with required artifacts."""
...
def history_mask_summary(history_path: Path) -> dict[str, Any]:
...
def main() -> int:
    ...
    audit_dir = run_dir / "stage3c_sparse_audit"
    ...
    run_command(audit_cmd, logs_dir / "stage3c_sparse_audit.log")
    ...
    manifest = {
        ...
        "history": history_mask_summary(run_dir / "secondU_history.json"),
        "metrics": str(metrics),
    ...
    gate = {
        ...
        "test_samples_recorded": metrics_data.get("samples") is not None,
        "sparse_metrics_recorded": "sparse_points" in metrics_data.get("secondU", {}),
    }
    gate["pass"] = all(gate.values())
    manifest["gate"] = gate
    save_json(manifest, run_dir / "stage3c_run_manifest.json")
    write_markdown(manifest, run_dir / "stage3c_run_manifest.md")
    print(manifest["gate"])
