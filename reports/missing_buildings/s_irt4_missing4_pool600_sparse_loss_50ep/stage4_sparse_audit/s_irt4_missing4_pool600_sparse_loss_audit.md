...
- 配置：`configs/s_irt4_missing4_pool600_sparse_loss.yaml`
...
## Loader / mask 对齐
- `train` 输入点数 `1..300`，loss mask 非零点数 `592..600`，输入点属于 loss mask 子集 `True`，输入 sparse 值与 target 最大误差 `0.0`
- `val` 输入点数 `1..296`，loss mask 非零点数 `591..600`，输入点属于 loss mask 子集 `True`，输入 sparse 值与 target 最大误差 `0.0`
- `test` 输入点数 `4..294`，loss mask 非零点数 `592..600`，输入点属于 loss mask 子集 `True`，输入 sparse 值与 target 最大误差 `0.0`
