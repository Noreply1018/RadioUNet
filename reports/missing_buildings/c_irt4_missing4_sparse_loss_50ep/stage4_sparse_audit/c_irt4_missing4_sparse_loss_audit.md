...
- sparse pool 配置：`300`
- 输入随机点数配置：`low=None, high=None`（上界为 numpy randint exclusive）
- loss mode：`sparse_mse`
- loss 分母：`num_samples_for_loss=300`
...
## Loader / mask 对齐
- `train` 输入点数 `0..0`，loss mask 非零点数 `296..300`，输入点属于 loss mask 子集 `True`，输入 sparse 值与 target 最大误差 `0.0`
- `val` 输入点数 `0..0`，loss mask 非零点数 `297..300`，输入点属于 loss mask 子集 `True`，输入 sparse 值与 target 最大误差 `0.0`
- `test` 输入点数 `0..0`，loss mask 非零点数 `296..300`，输入点属于 loss mask 子集 `True`，输入 sparse 值与 target 最大误差 `0.0`
